...
export async function localHasData(): Promise<boolean> {
	const [accounts, transactions, categories, categoryGroups, settings, goals] = await Promise.all([
		db.accounts.toArray(),
	...
	if (main.openingEnabled || (main.notes && main.notes.length > 0)) return true;
