...
test.describe('002 month charts', () => {
	...
	test('updates month totals after income and expense', async ({ page }) => {
		...
		await expect(page.getByTestId('month-net')).toHaveClass(/text-income/);
		await expect(page.getByTestId('month-footer-net')).toHaveClass(/text-income/);
		...
		await expect(
			page.getByTestId('income-category-chart').getByText('Income', { exact: true })
		).toBeVisible();
		await expect(
			page.getByTestId('category-chart').getByText('Expenses', { exact: true })
		).toBeVisible();
