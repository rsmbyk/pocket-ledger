# Tasks 136: Transactions row note primary + category icon

Accepted.

- [ ] Branch: `feat/136-transactions-row-note-category` after Accept
- [ ] Pass category icon slug into `TransactionListRow` from `ActivityTable`
- [ ] Left column: note primary / category+icon secondary; empty note and transfer rules
- [ ] Playwright: Transactions row order + icon; Recent unchanged
- [ ] Manual: YouTube + note row; empty note; transfer
