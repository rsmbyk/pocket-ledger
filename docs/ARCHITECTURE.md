...
SvelteKit **path** URLs (Spec 117 / 134 / 148): `/`, `/transactions` (`/activity` replace-navigates here), `/pockets`, `/pockets/:id`, `/categories`, `/more`. The service worker stays so signed-out still works offline after first load. Unknown paths fall back to the home shell (SPA). Hash bookmarks (`#/activity`) are not preserved.
