...
| Desktop shell                        | No Tauri; desktop-first dashboard shell (inset sidebar + KPI home + wide stage, spec 013). On xl, Home is two equal scrolling columns (238); pocket details is three (235); those columns use a flex height chain so they actually scroll and card elevation is not clipped (239); Transactions/Plans filters are a content-sized right card beside chrome+list (234). |
