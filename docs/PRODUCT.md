...
| Pockets                              | User-managed sub-accounts (**Pockets** nav item); `Main` is pinned first, never deleted, always the default for new transactions; non-Main pockets are user-reorderable (drag), renameable, and deletable once empty (spec 070); click a card for `/pockets/:id` details (spec 148); on xl, details is three equal scrolling columns (235); list cards are three-column (full-height grip column / name-description-goal / amount), hover-highlight, mute Main while dragging, no list edit/delete/clear-goal (specs 149, 151) |
| Pocket opening balance               | Each pocket has an opening balance + as-of date that seeds its derived running balance (spec 071)                                                                                                                                                                                   |
| Month Opening / Ending               | Home month Opening = sum of each pocket’s balance at month-start, inferred by walking txs backward/forward from that pocket’s opening as-of; Ending = Opening + Net (spec 110). Pocket details Ending = Opening + Net + TransferNet (in − out principal; spec 242).                                                                                                      |
| Goals                                | Many live Dexie rows per pocket (description, target, optional date); Drop instead of Clear; details Goals card + dedicated form (spec 152); progress from ledger balance; active chrome is amounts / date / right-aligned % / bar (170), fill expense→yellow@70→income in 10% steps (171) |
| Plans                                | Confirm-before-ledger reminders (not money until accept-mode Save). Nav **Plans** at `/plans`. Home shows a 1-week window card; pocket details lists that pocket’s Plans; Repeat Once / Weekly / Monthly (specs 223–224). Desktop sidebar collapses to an icon rail (226). On xl, Transactions and Plans are left chrome+list and a content-sized filter card (234). Plans Filters are type, category, and pocket (244). |
| Transfers                            | Move money between pockets as a single `transfer` transaction (source → dest, amount sent, optional admin fee, optional note); shown as a neutral row; fee counts as expense under synthetic **Admin Fee** (spec 073 / 106). Expenses may also carry an optional admin fee (spec 174) or pick Admin Fee as the category (237); income never does. Pocket details month summary shows Transfer in/out and includes TransferNet in Ending (spec 242); Home does not. |
| Currency                             | One display ISO (`displayCurrency`, default `IDR`); Settings picker is ISO (monospace) + name (spec 161); Save writes every pocket `currencyLabel` (spec 155)                                                                                                                                                           |
| Budgets                              | Many live Dexie rows per pocket (applies to pocket or expense categories, limit, hard limit, ongoing/monthly, start date). Details card between Plans and Goals; used from `startOn`/`effectiveStartOn` to today; bar is the color mirror of goals (spec 246). Hard limit refuses a tx that would exceed; otherwise warn and still post. |
| Multi-currency / FX                  | None                                                                                                                                                                                                                                                                                |
| UX                                   | **Desktop-first dashboard chrome**, responsive down to mobile (inset sidebar → sheet + stacked layouts below `sm`; other layouts still `md`). Session-unknown boot is a spinner (221); the shell keeps nav/header and shows destination-shaped skeletons until the ledger is ready (245). Installed PWA locks pinch/double-tap zoom (252); browser tabs stay zoomable.                                                                                                                                                                  |
| Features (shipped)                   | Charts, export — specs 001–008; net worth UI removed (059); recurring removed (087); Pockets nav + CRUD, per-pocket opening balance, per-pocket goals, transfers, Transactions pocket filter, row pocket labels — specs 070–077 / 134; month Opening from pocket openings — spec 110; pocket details dashboard — spec 148; Pockets list card states — spec 149; pocket grip column — spec 151; Plans (223–224); form Close → Cancel (225); sidebar icon rail (226); pocket budgets (246) and form/details chrome (247) with sticky groups, unique scope, and list preview (248); duplicate Applies-to is refused on Save with a field error (249) |
| Categories                           | Stock catalog in the app bundle (overlay in Dexie: custom groups/categories, hidden stock ids, group order). List per group; hide instead of delete; searchable grouped form picker (spec 123). Custom icon always `tag`. Uncategorized `circle-dashed`; Admin Fee `percent`. Custom category rename is a kind-tinted modal (spec 150). |
| Tests                                | Vitest + Playwright from the start                                                                                                                                                                                                                                                  |
| Process                              | Spec-Driven Development + TDD + Git Flow (develop integration, PRs into develop; ADR 0009)                                                                                                                                                                                                                                         |
| Desktop shell                        | No Tauri; desktop-first dashboard shell (inset sidebar + KPI home + wide stage, spec 013). On xl, Home is two equal scrolling columns (238); pocket details is three (235); those columns use a flex height chain so they actually scroll and card elevation is not clipped (239); Transactions/Plans filters are a content-sized right card beside chrome+list (234) with a Filters title and even header/footer padding (240). |
...
**Idle after:** 5 / 10 / 15 / **30** min (default 30). **When I leave this tab:** default **off**.
...
- Import: prompt file passphrase; wrong passphrase stays on the confirm with an error and keeps the chosen file (166). Full replace (as spec 003); then wrap or store raw DEK locally. Choose file button (168).
...
- Catch-up: pull on unlock, then PUT local rows with no `syncRevs` (pockets, custom overlay, txs, plans, goals, budgets, allowlisted settings). Pull after successful save, **30s** poll while visible and unlocked. **No offline queue** (save fails without network).
