...
| Transfers                            | Move money between pockets as a single `transfer` transaction (source → dest, amount sent, optional admin fee, optional note); shown as a neutral row; fee counts as expense under synthetic **Admin Fee** (spec 073 / 106). Expenses may also carry an optional admin fee (spec 174); income never does. |
...
| Desktop shell                        | No Tauri; desktop-first dashboard shell (inset sidebar + KPI home + wide stage, spec 013). On xl, Transactions/Plans filters are a content-sized right card beside chrome+list (234).                                                                                                  |
